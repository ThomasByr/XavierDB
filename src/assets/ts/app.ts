...
class Smoother {
  private buf: number[] = [];
  constructor(private window: number, private alpha = 0.35) {}
...
interface ClientNode {
  name: string; id: string; blocked: boolean; rps: number; p50_ms: number;
  total_requests: number; last_seen_ms: number; rps_history: number[];
}
interface AppNode {
  app: string; blocked: boolean; weight: number; rps: number; p50_ms: number; limit: number | null;
  rps_history: number[]; names: ClientNode[]; breakdown?: any;
}
interface Metrics {
  ts: number;
  config: { poll_seconds: number; theme: string; graph_smoothing: number; cfg_version: number; perms_version: number; health_ttl_seconds: number; multiplier: number };
  system: { cpu_pct: number; mem_pct: number; mem_used_mb: number; mem_total_mb: number; disk_pct: number; disk_used_mb: number; disk_total_mb: number; net_rx_kbps: number; net_tx_kbps: number; uptime_s: number; ts_ms: number };
  qps: number;
  health: any;
  apps: AppNode[];
...
let systemHistory: Record<string, number[]> = { cpu: [], mem: [], disk: [], rx: [], tx: [] };
...
async function initLogin() {
  const doLogin = async () => {
    ...
    try {
      await api("/login", { method: "POST", body: JSON.stringify({ username: user, password: pass }) });
...
const chartDefs: ChartDef[] = [
  { key: "cpu", label: "CPU", color: "--primary", raw: (s) => s.cpu_pct, fmt: (s) => fmtNum(s.cpu_pct, 0) + "%", sub: () => "of all cores" },
  { key: "mem", label: "Memory", color: "--secondary", raw: (s) => s.mem_pct, fmt: (s) => fmtNum(s.mem_pct, 0) + "%", sub: (s) => `${fmtBytes(s.mem_used_mb)} / ${fmtBytes(s.mem_total_mb)}` },
  { key: "disk", label: "Disk", color: "--warning", raw: (s) => s.disk_pct, fmt: (s) => fmtNum(s.disk_pct, 0) + "%", sub: (s) => `${fmtBytes(s.disk_used_mb)} / ${fmtBytes(s.disk_total_mb)}` },
  { key: "rx", label: "Download", color: "--primary", raw: (s) => s.net_rx_kbps, fmt: (s) => fmtNum(s.net_rx_kbps, 0), sub: () => "KB/s in" },
  { key: "tx", label: "Upload", color: "--secondary", raw: (s) => s.net_tx_kbps, fmt: (s) => fmtNum(s.net_tx_kbps, 0), sub: () => "KB/s out" },
];
...
function renderOverview() {
  ...
  for (const d of chartDefs) {
    ...
    const canvas = el("canvas", { width: "200", height: "48" }) as HTMLCanvasElement;
...
function renderOverviewData(m: Metrics) {
  ...
  if (chips) {
    ...
    const cards: [string, string, string][] = [
      ...
      ["MongoDB", health ? "reachable" : "DOWN", `ping ${fmtNum(m.health.mongodb?.ping_latency_ms ?? 0, 1)} ms`],
...
interface Rule { actions: string[]; databases: string[]; collections: string[]; source?: string }
interface NamePerm { name: string; allow: Rule[]; deny: Rule[]; effective?: EffectiveRule[]; delete?: boolean }
...
function renderClients() {
  ...
  v.append(el("div", { class: "card" }, [
    ...
    el("div", { class: "toolbar" }, [
      el("input", { id: "clients-search", type: "text", placeholder: "filter by name or app…", style: "max-width:240px;padding:8px 12px;border:1px solid var(--outline);border-radius:8px;background:var(--surface);color:var(--on-surface);font-size:13px;outline:none" }),
  ...
  v.append(el("div", { class: "card" }, [
    el("h3", {}, ["Adaptive limits"]),
    el("div", { id: "limits-view" }),
  ]));
  const cursorsView = el("div", { id: "cursors-view" });
  const cursorsCard = el("div", { class: "card", id: "cursors-card" }, [
    el("h3", {}, [
      "Live cursors",
      el("button", { id: "cursors-toggle", class: "btn btn-outline btn-small", style: "margin-left:auto", title: "debugging only" }, [hideCursors ? "Show" : "Hide"]),
    ]),
    cursorsView,
  ]);
  ...
  search.oninput = () => { clientsSearch = search.value; applyClientsFilter(); };
...
function applyClientsFilter() {
  ...
  if (!q) {
    for (const n of Array.from(tree.querySelectorAll<HTMLElement>("[data-name]"))) n.classList.remove("hidden");
    for (const n of Array.from(tree.querySelectorAll<HTMLElement>("[data-app]"))) n.classList.remove("hidden");
    return;
  }
  for (const node of Array.from(tree.querySelectorAll<HTMLElement>("[data-app]"))) {
    ...
    if (!app) continue;
    const match = !q || app.app.toLowerCase().includes(q) || app.names.some((n) => n.name.toLowerCase().includes(q));
...
async function loadPermsData() {
  ...
  } catch (e: any) { snack(e.message); return; }
  if (!permsData) return;
...
function mergedApps(m: Metrics) {
  ...
  for (const p of permsData?.apps ?? []) {
    ...
    if (!a) {
      a = { app: p.app, blocked: false, weight: 1, rps: 0, p50_ms: 0, limit: null, rps_history: [], names: [], breakdown: undefined };
      byId.set(p.app, a);
      out.push(a);
    }
    for (const n of p.names) {
      if (!a.names.some((x) => x.name === n.name)) {
        a.names.push({ name: n.name, id: `${n.name}@${p.app}`, blocked: false, rps: 0, p50_ms: 0, total_requests: 0, last_seen_ms: 0, rps_history: [] });
      }
    }
...
function renderClientsData(m: Metrics) {
  ...
  for (const a of apps) {
    ...
    if (!node) { node = buildAppNode(a); tree.append(node); }
    updateAppNode(node, a, q);
    updateNames(node, a, q);
  }
  for (const [k, node] of nodes) if (!seen.has(k)) node.remove();
  if (apps.length === 0) tree.append(el("div", { class: "empty-note" }, ["no apps yet — add one with “+ add app”, or edit authorized_keys.yml"]));
  renderLimits(m);
  renderCursors(m);
}
...
function updateNames(appNode: HTMLElement, a: AppNode, q: string) {
  ...
  for (const c of Array.from(ul.children) as HTMLElement[]) if (c.dataset.name) nameNodes.set(c.dataset.name, c);
  const seenN = new Set<string>();
  for (const n of a.names) {
    seenN.add(n.id);
    let nnode = nameNodes.get(n.id);
    if (!nnode) { nnode = buildNameNode(a.app, n); ul.append(nnode); }
...
function buildAppNode(a: AppNode): HTMLElement {
  ...
  row.append(el("span", { class: "badge info", "data-role": "perm-badge", style: "display:none", title: "databases granted in authorized_keys.yml" }));
  const meta = el("span", { class: "tree-meta" }, [
    el("span", { class: "weight-label", "data-role": "app-weight", title: "adaptive-limit weight (×0.1 – ×10) — click to adjust" }, ["×1.0"]),
    el("canvas", { class: "sparkline", "data-role": "app-spark", width: "70", height: "22", style: "width:70px;height:22px" }),
    el("span", { "data-role": "app-rps" }),
    el("span", { "data-role": "app-limit" }),
    el("span", { class: "muted", "data-role": "app-p50" }),
  ]);
  const btn = el("button", { class: "btn btn-small btn-outline blockbtn", "data-role": "blockbtn" });
  btn.onclick = async (e) => {
    e.stopPropagation();
    const cur = lastMetrics?.apps.find((x) => x.app === node.dataset.app);
    const was = cur?.blocked ?? false;
    if (!was && !(await confirmDialog(`Block app ${node.dataset.app}?`, "All names under this app will get 403 BLOCKED.", "Block"))) return;
    try {
      await api(`/${was ? "unblock" : "block"}`, { method: "POST", body: JSON.stringify({ id: node.dataset.app }) });
      snack(`app ${was ? "unblocked" : "blocked"}`);
      poll();
    } catch (err: any) { snack(err.message); }
  };
  meta.append(btn);
  row.append(meta);
  ...
  panel.append(el("div", { "data-role": "adap", class: "perms-summary", style: "display:none;margin-top:8px" }));
  panel.append(el("hr"));
  // token rotation
  const tok = el("div", { class: "row", style: "gap:8px" });
  const tokIn = el("input", { type: "password", placeholder: "new shared token (min 8 chars)", style: "flex:1;min-width:160px;max-width:240px;padding:7px 10px;font-size:12px;border:1px solid var(--outline);border-radius:6px;background:var(--surface);color:var(--on-surface);outline:none" });
  const tokBtn = el("button", { class: "btn btn-outline btn-small" }, ["Rotate token"]);
  tokBtn.onclick = async () => {
    const t = tokIn.value;
    if (t.length < 8) { snack("token too short (min 8 chars)"); return; }
  ...
  const chkDb = el("input", { type: "text", placeholder: "database (glob ok)", style: "width:130px;padding:6px 10px;font-size:12px;border:1px solid var(--outline);border-radius:6px;background:var(--surface);color:var(--on-surface);outline:none" });
  const chkColl = el("input", { type: "text", placeholder: "collection", style: "width:110px;padding:6px 10px;font-size:12px;border:1px solid var(--outline);border-radius:6px;background:var(--surface);color:var(--on-surface);outline:none" });
...
function buildNameNode(app: string, n: ClientNode): HTMLElement {
  ...
  row.append(el("span", { class: "badge", "data-role": "blockbadge" }));
  const cv = el("canvas", { class: "sparkline", width: "70", height: "22", style: "width:70px;height:22px" });
  row.append(cv);
  const meta = el("span", { class: "tree-meta" }, [
  ...
  btn.onclick = async (e) => {
    ...
    const was = cur?.blocked ?? false;
    if (!was && !(await confirmDialog(`Block ${n.id}?`, "This name will get 403 BLOCKED on every request.", "Block"))) return;
    try {
      await api(`/${was ? "unblock" : "block"}`, { method: "POST", body: JSON.stringify({ id: n.id }) });
  ...
  delBtn.onclick = async (e) => {
    e.stopPropagation();
    if (!(await confirmDialog(`Delete ${n.name}?`, "The name falls back to the app permissions.", "Delete"))) return;
    const real = permsData?.apps.find((x) => x.app === app)?.names.find((x) => x.name === n.name);
    if (real) { real.delete = true; queuePermsSave(); }
...
function appEntry(app: string): AppPerm {
  ...
  if (!d) { d = { app, token_set: false, allow: [], deny: [], names: [] }; detachApps.set(app, d); }
  return d;
}
...
function nameEntry(app: string, name: string): NamePerm {
  ...
  if (!d) { d = { name, allow: [], deny: [] }; detachNames.set(key, d); }
  return d;
}
...
function refreshAppNode(node: HTMLElement) {
  ...
  if (badge) {
    if (pats.length) { badge.style.display = ""; badge.textContent = `${pats.length} db${pats.length === 1 ? "" : "s"}`; }
    else badge.style.display = "none";
  }
  renderPermWidget(widget, entry.allow, entry.deny, { scope: "app", eff: entry.effective ?? [] }, () => queuePermsSave());
...
function refreshNameNode(node: HTMLElement) {
  ...
  renderPermWidget(widget, entry.allow, entry.deny, { scope: "name", eff: entry.effective ?? [] }, () => queuePermsSave());
...
function updateAppNode(node: HTMLElement, a: AppNode, q: string) {
  ...
  if (bb) { bb.className = "badge " + (a.blocked ? "bad" : "ok"); bb.textContent = a.blocked ? "BLOCKED" : "active"; }
  ...
  node.classList.toggle("hidden", !!q && !a.app.toLowerCase().includes(q) && !a.names.some((n) => n.name.toLowerCase().includes(q)));
}
...
function updateNameNode(node: HTMLElement, n: ClientNode, q: string, appMatch: boolean) {
  ...
  if (bb) { bb.className = "badge " + (n.blocked ? "bad" : "ok"); bb.textContent = n.blocked ? "BLOCKED" : "active"; }
...
function closeWeightPop() {
  weightPopEl?.remove();
  weightPopEl = null;
  if (weightPopDoc) { document.removeEventListener("mousedown", weightPopDocHandler); weightPopDoc = false; }
}
...
function openWeightPop(anchor: HTMLElement, app: string, cur: number) {
  ...
  const slider = el("input", { type: "range", min: "0.1", max: "10", step: "0.1", value: String(cur) }) as HTMLInputElement;
  slider.addEventListener("input", () => { val.textContent = "×" + parseFloat(slider.value).toFixed(1); });
  slider.addEventListener("change", async () => {
    const w = parseFloat(slider.value);
    try {
      await api("/app_weight", { method: "POST", body: JSON.stringify({ id: app, weight: w }) });
...
function entriesFromRules(allow: Rule[], deny: Rule[]): PermEntry[] {
  ...
  const add = (r: Rule, st: ActState) => {
    for (const db of r.databases) {
      let e = map.get(db);
      if (!e) { e = { pattern: db, isGlob: /[*?]/.test(db), db: {}, colls: new Map() }; map.set(db, e); }
...
function effVerdict(eff: EffectiveRule[], action: string, db: string, coll: string, appOnly: boolean): "allow" | "deny" | null {
  for (const r of eff) {
...
interface WidgetUIState { expanded: Set<string>; search: string }
...
function renderPermWidget(cont: HTMLElement, allow: Rule[], deny: Rule[], ctx: PermCtx, onSave: () => void) {
  ...
  const matches = (q: string, name: string) => (hasWild(q) ? glob(q, name) : name.toLowerCase().includes(q.toLowerCase()));
  const commit = () => { applyEntries(allow, deny, entries); onSave(); };
  ...
  const input = el("input", { type: "text", placeholder: "search… or add a database / glob pattern (e.g. analytics_*)" });
  const addLabel = el("label", { class: "switch", title: "add the pattern as a permission entry (all actions allowed)" });
  ...
  const actBadge = (action: string, cls: string, title: string, onClick: (() => void) | null): HTMLElement => {
  ...
  const dbDisp = (action: string, own: ActState | null, lock: string, pat: string): { cls: string; title: string } => {
    if (lock) {
      const gv = globVerdict(action);
      const deny = own === "d" || gv === "d";
      const allow = !deny && (own === "a" || gv === "a");
      return deny ? { cls: "inherit-deny", title: `denied by pattern ${lock}` }
        : allow ? { cls: "inherit-allow", title: `allowed by pattern ${lock}` }
    ...
    if (ctx.scope === "name") {
      ...
      if (v === "allow") return { cls: "inherit-allow", title: "inherits app — allowed — click to override" };
      if (v === "deny") return { cls: "inherit-deny", title: "inherits app — denied — click to override" };
    }
    return { cls: "none", title: "no rule — denied by default — click to allow" };
  };
  const collDisp = (action: string, own: ActState | null, lock: string, pat: string, coll: string): { cls: string; title: string } => {
    ...
    if (lock) {
      return v === "d" ? { cls: "inherit-deny", title: `denied by pattern ${lock}` }
        : v === "a" ? { cls: "inherit-allow", title: `allowed by pattern ${lock}` }
        : { cls: "none", title: `not covered by pattern ${lock}` };
    }
    const base = v === "allow" ? "database allows it" : v === "deny" ? "database denies it" : "nothing grants it";
  ...
  const rowBadges = (pat: string, entry: PermEntry | null | undefined, lock: string, dbLevel: boolean, coll?: string): HTMLElement => {
    ...
    for (const action of ACTIONS) {
      const own = entry ? (dbLevel ? entry.db[action] ?? null : entry.colls.get(coll!)?.[action] ?? null) : null;
      const disp = dbLevel ? dbDisp(action, own, lock, pat) : collDisp(action, own, lock, pat, coll!);
      const onClick = lock ? null : () => {
        let e = entry;
        if (!e) { e = { pattern: pat, isGlob: false, db: {}, colls: new Map() }; entries.push(e); }
        if (dbLevel) {
          const nv = next(e.db[action] ?? null);
          if (nv === null) delete e.db[action]; else e.db[action] = nv;
        } else {
          let cs = e.colls.get(coll!);
          if (!cs) { cs = {}; e.colls.set(coll!, cs); }
          const nv = next(cs[action] ?? null);
          if (nv === null) delete cs[action]; else cs[action] = nv;
        }
        commit();
        renderRows();
      };
  ...
  const renderColls = (entry: PermEntry | null | undefined, pat: string, lock: string): HTMLElement => {
    ...
    const rows: [string, boolean][] = [
      ...
      ...(entry ? [...entry.colls.keys()].filter((c) => !realColls.includes(c)).map((c): [string, boolean] => [c, false]) : []),
    ];
    for (const [coll, real] of rows) {
      const r = el("div", { class: "perm-row" + (real ? "" : " dim") });
      r.append(el("span", { class: "perm-name", title: real ? "" : "collection pattern from the permission file" }, [esc(coll)]));
      ...
      if (entry?.colls.has(coll)) {
        ...
        reset.onclick = (ev) => { ev.stopPropagation(); entry.colls.delete(coll); commit(); renderRows(); };
        r.append(reset);
      }
      box.append(r);
    }
    if (!lock) {
      ...
      const ai = el("input", { type: "text", placeholder: "collection or glob (e.g. log_*)" });
      const ab = el("button", { class: "mini-btn add", title: "add a collection override (all actions allowed)" }, ["+ add"]);
      const doAdd = () => {
        const c = ai.value.trim();
        if (!c) return;
        let e = entry;
        if (!e) { e = { pattern: pat, isGlob: false, db: {}, colls: new Map() }; entries.push(e); }
        if (e.colls.has(c)) { snack(`override for “${c}” already exists`); return; }
      ...
      ai.addEventListener("keydown", (ev) => { if (ev.key === "Enter") { ev.stopPropagation(); doAdd(); } });
      add.append(ai, ab);
      box.append(add);
    }
    return box;
  ...
  const renderRows = () => {
    ...
    const extras = entries.filter((e) => !e.isGlob && !dbList.some((d) => d.name === e.pattern)).map((e) => e.pattern);

    for (const [pat, real] of [...realDbs.map((d): [string, boolean] => [d.name, true]), ...extras.map((p): [string, boolean] => [p, false])]) {
      if (q && !matches(q, pat)) continue;
      const entry = entryFor(pat);
      const lock = lockedBy(pat);
      const row = el("div", { class: "perm-row" + (real ? "" : " dim") });
      const caret = el("span", { class: "caret" + (wui.expanded.has(pat) ? "" : " closed"), title: "collections" }, ["▸"]);
      caret.onclick = (ev) => {
        ev.stopPropagation();
      ...
      row.append(caret);
      row.append(el("span", { class: "perm-name", title: real ? "" : "not in MongoDB — pattern from the permission file" }, [esc(pat)]));
      row.append(rowBadges(pat, entry, lock, true));
      if (lock) row.append(el("span", { class: "perm-lock", title: `controlled by pattern ${lock} — edit or delete it to change this database` }, ["🔒"]));
      dbsBox.append(row);
      if (wui.expanded.has(pat)) dbsBox.append(renderColls(entry, pat, lock));
    }
    if (dbsBox.children.length === 0) {
      dbsBox.append(el("div", { class: "muted", style: "padding:4px 6px" }, [
        q ? `no database matches “${esc(q)}”` : "no databases yet — type a pattern above and toggle it to grant access",
      ]));
    } else if (dbListUnavailable) {
      dbsBox.append(el("div", { class: "muted", style: "padding:4px 6px" }, ["MongoDB unreachable — listing permission-file patterns only"]));
    }
    ...
    if (globs.length) {
      ...
      for (const e of globs) {
        ...
        const caret = el("span", { class: "caret" + (wui.expanded.has(e.pattern) ? "" : " closed"), title: "collections" }, ["▸"]);
        caret.onclick = (ev) => {
          ev.stopPropagation();
        ...
        reset.onclick = (ev) => { ev.stopPropagation(); e.db = {}; e.colls.clear(); commit(); renderRows(); };
        const del = el("button", { class: "mini-btn", title: "delete pattern" }, ["✕"]);
        del.onclick = (ev) => { ev.stopPropagation(); entries.splice(entries.indexOf(e), 1); commit(); renderRows(); };
...
function renderEffective(cont: HTMLElement, eff: EffectiveRule[], appScope: boolean) {
  cont.innerHTML = "";
  if (!eff.length) {
    cont.append(el("div", { class: "muted" }, [
      appScope ? "no rules — every request is denied by default" : "no own rules — inherits the app permissions",
    ]));
    return;
  }
  const t = el("table", { class: "data table-sm" });
  t.innerHTML = "<thead><tr><th>source</th><th>actions</th><th>databases</th><th>collections</th></tr></thead>";
  const tb = el("tbody");
  for (const r of eff) {
    const tr = el("tr");
    tr.append(el("td", {}, [el("span", { class: "badge " + (r.source.endsWith("deny") ? "bad" : "info") }, [r.source])]));
...
function runCheck(db: string, coll: string, out: HTMLElement, eff: EffectiveRule[]) {
  ...
  for (const action of ACTIONS) {
    let verdict = false;
    for (const r of eff) {
      if (r.actions.includes(action) && r.databases.some((p) => glob(p, db)) && r.collections.some((p) => glob(p, coll))) {
        verdict = !r.source.endsWith("deny");
        break;
      }
    }
    out.append(el("span", { class: "badge " + (verdict ? "ok" : "bad") }, [`${action} ${verdict ? "✓" : "✗"}`]));
  }
}
...
function renderLimits(m: Metrics) {
  ...
  if (!withBd.length) {
    cv.append(el("div", { class: "muted" }, ["no adaptive limit state yet — appears once apps send traffic"]));
    return;
  }
  const t = el("table", { class: "data table-sm" });
  t.innerHTML = "<thead><tr><th>app</th><th>limit</th><th>enforced</th><th>rate</th><th>p50</th><th>lat_err</th><th>pressure</th><th>shrink</th></tr></thead>";
  const tb = el("tbody");
...
function renderCursors(m: Metrics) {
  ...
  const t = el("table", { class: "data table-sm" });
  t.innerHTML = "<thead><tr><th>id</th><th>collection</th><th>created</th><th>last used</th><th>pages</th></tr></thead>";
  const tb = el("tbody");
...
function queuePermsSave() {
  permSaveChain = permSaveChain
    .then(savePerms)
    .catch((e) => snack("permissions: " + (e.message || e)));
}

async function savePerms() {
  if (!permsData) return;
  type NamePayload = { name: string; allow?: Rule[]; deny?: Rule[]; delete?: boolean };
  type AppPayload = { app: string; allow: Rule[]; deny: Rule[]; names: NamePayload[]; delete?: boolean; set_token?: string };
  const apps: AppPayload[] = permsData.apps.map((a) => ({
    app: a.app, allow: a.allow, deny: a.deny,
    names: a.names.map((n) => (n.delete ? { name: n.name, delete: true } : { name: n.name, allow: n.allow, deny: n.deny })),
    delete: a.delete, set_token: a.set_token,
  }));
  ...
  for (const [app, d] of detachApps) {
    if (d.allow.length || d.deny.length || d.set_token) {
      apps.push({
        app, allow: d.allow, deny: d.deny,
        names: detachedNamesFor(app).map((n) => ({ name: n.name, allow: n.allow, deny: n.deny })),
  ...
  for (const a of permsData.apps) {
    ...
    if (extra.length) {
      apps.find((x) => x.app === a.app)!.names.push(...extra.map((n) => ({ name: n.name, allow: n.allow, deny: n.deny })));
    }
  }
...
function addAppDialog() {
  ...
  box.append(el("div", { class: "field" }, [el("label", {}, ["app_id"]), el("input", { id: "new-app-id", type: "text", placeholder: "e.g. provider2", spellcheck: "false" })]));
  box.append(el("div", { class: "field" }, [el("label", {}, ["shared token (min 8 chars)"]), el("input", { id: "new-app-token", type: "password" })]));
  const actions = el("div", { class: "dialog-actions" });
  const cancel = el("button", { class: "btn btn-text" }, ["Cancel"]);
  ...
  ok.onclick = async () => {
    ...
    if (!id || tok.length < 8) { snack("app_id and a token of min 8 chars are required"); return; }
    if (!permsData) permsData = { version: 0, apps: [] };
    permsData.apps.push({ app: id, token_set: false, allow: [], deny: [], names: [], set_token: tok });
    try {
      await savePerms();
...
function renderConfigForm() {
  ...
  if (!form) return;
  form.innerHTML = "";
  $("#cfg-meta").textContent = `version ${configData.version} · undo ${configData.undo_available ? "available" : "—"} · redo ${configData.redo_available ? "available" : "—"}`;

  interface CfgField {
    path: string; label: string; kind: "range" | "text" | "select";
  ...
  configData.history.forEach((h: any, i: number) => {
    ...
    row.onclick = async () => {
      if (!(await confirmDialog("Revert config?", `Restore the state before "${h.desc}"? Changes after it will be discarded.`, "Revert"))) return;
      try {
        await api("/config/revert", { method: "POST", body: JSON.stringify({ index: i }) });
        snack("reverted");
        loadConfig();
      } catch (e: any) { snack(e.message); }
    };
  ...
  $("#cfg-reload").onclick = async () => { await api("/config/reload", { method: "POST" }); loadConfig(); snack("reloaded"); };
  $("#cfg-reset").onclick = async () => {
    if (!(await confirmDialog("Reset to defaults?", "All settings return to their default values (history kept).", "Reset"))) return;
    await api("/config/reset", { method: "POST" });
    loadConfig();
    snack("reset to defaults");
  };
  $("#cfg-export").onclick = async () => {
    const r = await fetch("/dashboard/api/config/export", { credentials: "same-origin" });
    if (!r.ok) {
  ...
  $("#cfg-import").onclick = () => {
    ...
    box.append(el("textarea", { id: "cfg-import-text", style: "width:100%;height:180px;font-family:monospace;font-size:12px" }));
    const actions = el("div", { class: "dialog-actions" });
    const cancel = el("button", { class: "btn btn-text" }, ["Cancel"]);
    cancel.onclick = () => $("#dialog").classList.add("hidden");
    ...
    ok.onclick = async () => {
      try {
        ...
        await api("/config/import", { method: "POST", body: JSON.stringify({ config: parsed }) });
        $("#dialog").classList.add("hidden");
        loadConfig();
        snack("imported");
      } catch (e: any) { snack("import failed: " + e.message); }
    };
    actions.append(cancel, ok);
    box.append(actions);
    $("#dialog").classList.remove("hidden");
