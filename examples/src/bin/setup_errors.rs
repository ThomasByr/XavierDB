//! setup_errors — grant the app used by the `errors` showcase.
//!
//! Uses the dashboard API: login as admin, then POST /dashboard/api/perms to
//! create (or refresh) the `xdb-errors` app with a shared token and a
//! deliberately narrow rights set: GET only on db1, and write access only on
//! the `xdb_errors` database. The showcase walks the error contract
//! (401/403/404/409). Safe to re-run: only this app is touched (merge
//! semantics), its allow/deny are replaced wholesale, and the token is
//! rehashed on every run.
//!
//! Usage:
//!   cargo run --manifest-path examples/Cargo.toml --bin setup_errors -- \
...
fn main() {
            ...
            "apps": [{
                ...
                "allow": [
                    ...
                    { "actions": ["POST", "PUT", "PATCH", "DELETE"],
                      "databases": ["xdb_errors"], "collections": ["*"] }
                ],
                "deny": [],
                "set_token": token,
            }]
    ...
    println!("perms ok: app {APP}, token set, GET on db1 + writes on xdb_errors");
}
