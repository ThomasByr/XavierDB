//! setup_query — grant the app used by the `query` showcase.
//!
//! Uses the dashboard API: login as admin, then POST /dashboard/api/perms to
//! create (or refresh) the `xdb-query` app with a shared token and GET+POST
//! rights on the `xdb_query` database. Safe to re-run: only this app is
//! touched (merge semantics), its allow/deny are replaced wholesale, and the
//! token is rehashed on every run.
//!
//! Usage:
//!   cargo run --manifest-path examples/Cargo.toml --bin setup_query -- \
...
fn main() {
    ...
    let resp = agent
        ...
        .send_json(json!({
            "apps": [{
                "app": APP,
                "allow": [
                    { "actions": ["GET", "POST"], "databases": ["xdb_query"], "collections": ["*"] }
                ],
                "deny": [],
                "set_token": token,
            }]
    ...
    println!("perms ok: app {APP}, token set, GET+POST on xdb_query");
}
