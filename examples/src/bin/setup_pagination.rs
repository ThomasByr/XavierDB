//! setup_pagination — grant the app used by the `pagination` showcase.
//!
//! Uses the dashboard API: login as admin, then POST /dashboard/api/perms to
//! create (or refresh) the `xdb-pagination` app with a shared token and
//! GET+POST rights on the `xdb_pagination` database. Safe to re-run: only
//! this app is touched (merge semantics), its allow/deny are replaced
//! wholesale, and the token is rehashed on every run.
//!
//! Usage:
//!   cargo run --manifest-path examples/Cargo.toml --bin setup_pagination -- \
...
fn main() {
    ...
    let resp = agent
        ...
        .send_json(json!({
            "apps": [{
                "app": APP,
                "allow": [
                    { "actions": ["GET", "POST"], "databases": ["xdb_pagination"], "collections": ["*"] }
                ],
                "deny": [],
                "set_token": token,
            }]
    ...
    println!("perms ok: app {APP}, token set, GET+POST on xdb_pagination");
}
