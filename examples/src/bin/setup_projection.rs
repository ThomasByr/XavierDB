//! setup_projection — grant the app used by the `projection` showcase.
//!
//! Uses the dashboard API: login as admin, then POST /dashboard/api/perms to
//! create (or refresh) the `xdb-projection` app with a shared token and
//! GET+POST rights on the `xdb_projection` database. Safe to re-run: only
...
fn main() {
    ...
    let resp = agent
        ...
        .send_json(json!({
            "apps": [{
                "app": APP,
                "allow": [
                    { "actions": ["GET", "POST"], "databases": ["xdb_projection"], "collections": ["*"] }
                ],
                "deny": [],
                "set_token": token,
            }]
    ...
    println!("perms ok: app {APP}, token set, GET+POST on xdb_projection");
}
