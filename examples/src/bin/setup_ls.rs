//! setup_ls — grant the app used by the `ls` showcase.
//!
//! Uses the dashboard API: login as admin, then POST /dashboard/api/perms to
//! create (or refresh) the `xdb-ls` app with a shared token and GET rights on
//! every database (`*` glob). Safe to re-run: only this app is touched
//! (merge semantics), its allow/deny are replaced wholesale, and the token is
//! rehashed on every run.
//!
//! Usage:
//!   cargo run --manifest-path examples/Cargo.toml --bin setup_ls -- \
...
fn main() {
    ...
    let resp = agent
        ...
        .send_json(json!({
            "apps": [{
                "app": APP,
                "allow": [
                    { "actions": ["GET"], "databases": ["*"], "collections": ["*"] }
                ],
                "deny": [],
                "set_token": token,
            }]
    ...
    println!("perms ok: app {APP}, token set, GET on all databases");
}
