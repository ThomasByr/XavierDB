//! setup_indexes — grant the app used by the `indexes` showcase.
//!
//! Uses the dashboard API: login as admin, then POST /dashboard/api/perms to
//! create (or refresh) the `xdb-indexes` app with a shared token and
//! GET+POST+INDEX rights on the `xdb_indexes` database. INDEX is the
//! dedicated schema-level action for ensure/drop on /q/{db}/{coll}/indexes
//! (reading indexes uses plain GET). Safe to re-run: only this app is
//! touched (merge semantics), its allow/deny are replaced wholesale, and
//! the token is rehashed on every run.
//!
//! Usage:
//!   cargo run --manifest-path examples/Cargo.toml --bin setup_indexes -- \
...
fn main() {
    ...
    let resp = agent
        ...
        .send_json(json!({
            "apps": [{
                "app": APP,
                "allow": [
                    { "actions": ["GET", "POST", "INDEX"], "databases": ["xdb_indexes"], "collections": ["*"] }
                ],
                "deny": [],
                "set_token": token,
            }]
    ...
    println!("perms ok: app {APP}, token set, GET+POST+INDEX on xdb_indexes");
}
