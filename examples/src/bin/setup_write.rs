//! setup_write — grant the app used by the `write` showcase.
//!
//! Uses the dashboard API: login as admin, then POST /dashboard/api/perms to
//! create (or refresh) the `xdb-write` app with a shared token and all five
//! actions on the `xdb_write` database. Safe to re-run: only this app is
//! touched (merge semantics), its allow/deny are replaced wholesale, and the
//! token is rehashed on every run.
//!
//! Usage:
//!   cargo run --manifest-path examples/Cargo.toml --bin setup_write -- \
...
fn main() {
            ...
            "apps": [{
                ...
                "allow": [
                    { "actions": ["GET", "POST", "PUT", "PATCH", "DELETE"],
                      "databases": ["xdb_write"], "collections": ["*"] }
                ],
                "deny": [],
                "set_token": token,
            }]
    ...
    println!("perms ok: app {APP}, token set, all actions on xdb_write");
}
