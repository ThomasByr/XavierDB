//! setup_pernames — grant the app used by the `pernames` showcase.
//!
//! Uses the dashboard API: login as admin, then POST /dashboard/api/perms to
//! create (or refresh) the `xdb-pernames` app. It gets NO app-level rights
//! (allow/deny empty — replaced wholesale on every run, so they must be sent
//! explicitly); access lives entirely in the per-name rules:
//!   reader -> GET  on xdb_pernames
//!   writer -> GET+POST on xdb_pernames
//! The token is rehashed on every run. Safe to re-run: only this app is
//! touched (merge semantics).
//!
//! Usage:
//!   cargo run --manifest-path examples/Cargo.toml --bin setup_pernames -- \
...
fn main() {
    ...
    let resp = agent
        ...
        .send_json(json!({
            "apps": [{
                ...
                "names": [
                    { "name": "reader",
                      "allow": [{ "actions": ["GET"], "databases": ["xdb_pernames"], "collections": ["*"] }],
                      "deny": [] },
                    { "name": "writer",
                      "allow": [{ "actions": ["GET", "POST"], "databases": ["xdb_pernames"], "collections": ["*"] }],
                      "deny": [] }
                ],
                "set_token": token,
            }]
    ...
    println!(
        "perms ok: app {APP}, token set, names reader(GET) + writer(GET,POST) on xdb_pernames"
    );
}
