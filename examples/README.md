...
| `setup_indexes`    | `indexes`    | `xdb-indexes` / `xdb_indexes`                   | the index lifecycle on GET/POST/DELETE `/q/{db}/{coll}/indexes`: list, ensure (201 / idempotent 200, unique, TTL), both 409 conflicts, drop (200 / 404 / 400 on `_id_`) |
