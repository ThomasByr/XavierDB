...
│       └── ts/                  # dashboard SPA source modules (zero-dep TS, esbuild-bundled):
│                                 # app.ts (entry) · core.ts · state.ts · rps-archive.ts · charts.ts
│                                 # mongo.ts · view-{overview,clients,config,logs}.ts · perm-widget.ts
